...
export function AddNews({reload,speciality}) {  
    ...
    async function Publish(){
      setPublishStatus(1);
      try{
        const docRef = await addDoc(collection(db, "PGupload","Notes","Note"), {
          Title: title,
    ...
    return (
      <Dialog onOpenChange={reset}>
        <DialogTrigger asChild>
          <Button variant="secondary"  className="gap-x-2"><MdAdd className="scale-125"/> Add Notes</Button>
        </DialogTrigger>
          ...
          <form>
            <div className="grid w-full items-center gap-5">
              <div className="flex flex-col space-y-2">
                ...
                <Input id="title" placeholder="Note Title" value={title} 
