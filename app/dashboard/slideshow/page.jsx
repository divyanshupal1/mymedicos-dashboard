    ...
    <div className="view w-full flex  justify-center h-full">
          {loading ?
            ...
            <div className="w-full gap-3 flex flex-col flex-wrap p-3">
             {docs.map((state,index)=><Link href={'/dashboard/slideshow/'+state} key={index}><div className="p-3 bg-secondary hover:bg-primary hover:text-white rounded-lg">{state}</div></Link>)}
            </div>
          }
      </div>
